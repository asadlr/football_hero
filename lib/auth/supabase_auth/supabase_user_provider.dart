...
class FootballHeroSupabaseUser extends BaseAuthUser {
  FootballHeroSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> footballHeroSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FootballHeroSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
